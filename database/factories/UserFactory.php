<?php

namespace Database\Factories;

use App\Models\Class1;
use App\Models\User;
use Illuminate\Database\Eloquent\Factories\Factory;
use Illuminate\Support\Str;

class UserFactory extends Factory
{
    /**
     * Define the model's default state.
     *
     * @return array
     */
    public function definition()
    {
        return [
            'lastName' => $this->faker->lastName(),
            'firstName' => $this->faker->firstName(),
            'email' => $this->faker->unique()->safeEmail(),
            'email_verified_at' => now(),
            'password' => '$2y$10$92IXUNpkjO0rOQ5byMi.Ye4oKoEa3Ro9llC/.og/at2.uheWG/igi', // password
            'remember_token' => Str::random(10),
            'verified' => $verified = $this->faker->randomElement([User::VERIFIED_USER,User::UNVERIFIED_USER]),
            'verification_token' => 'i.Ye4oKoEa3R',
            'image' => $this->faker->randomElement(['1.jpg', '2.jpg', '3.jpg']),
            'admin' => $admin = $this->faker->randomElement([User::ADMIN_USER,User::REGULAR_USER]),
            'role' => $role = $this->faker->randomElement([USER::STUDENT_USER, User::TEACHER_USER]),
            'class1_id' => $role == User::STUDENT_USER ? Class1::all()->random()->id : null
        ];
    }

    /**
     * Indicate that the model's email address should be unverified.
     *
     * @return \Illuminate\Database\Eloquent\Factories\Factory
     */
    public function unverified()
    {
        return $this->state(function (array $attributes) {
            return [
                'email_verified_at' => null,
            ];
        });
    }

    private function generateVerificationCode () {
        Str::random(40);
    }
}
